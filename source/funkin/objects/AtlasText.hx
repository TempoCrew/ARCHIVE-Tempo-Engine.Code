 ...
 */
class AtlasText extends flixel.group.FlxSpriteGroup.FlxTypedSpriteGroup<AtlasChar>
{
	...
	{
		return "InputItem, " + flixel.util.FlxStringUtil.getDebugString([
			flixel.util.FlxStringUtil.LabelValuePair.weak("x", x),
			flixel.util.FlxStringUtil.LabelValuePair.weak("y", y),
			flixel.util.FlxStringUtil.LabelValuePair.weak("text", text)
		]);
	}
}
...
private class AtlasFontData
{
	...
	public function new(name:AtlasFont)
	{
		...
		atlas = Paths.atlas.sparrow(Paths.image('fonts/${fontName.toLowerCase()}'));
		if (atlas == null)
		{
