...
#end
import engine.ui.preload.TempoPreloader as TP;

/**
 * Updater/Loader for `TempoPreloader.hx`
 */
@:access(engine.ui.preload.TempoPreloader)
class TempoPreloaderUpdater
{
	...
	public static function updateState(percent:Float, elapsed:Float):Float
	{
		...
		{
			case NotStarted:
				if (TP.instance.downloadingAssetsPercent > 0.0)
					TP.instance.currentState = Downloading;

				return percent;

			case Downloading:
				if (TP.instance.downloadingAssetsPercent >= 1.0
					|| (elapsed > Constants.PRELOADER_MIN_STAGE_TIME && TP.instance.downloadingAssetsComplete))
					TP.instance.currentState = Preloading;
			...
			case Preloading:
				...
				else if (Constants.PRELOADER_MIN_STAGE_TIME > 0)
				{
					...
					{
						if (TP.instance.preloadingPlayAssetsPercent < (elapsedPreloadingPlayAssets / Constants.PRELOADER_MIN_STAGE_TIME))
				...
				{
					if (TP.instance.preloadingPlayAssetsComplete)
						TP.instance.currentState = Initializing;
				}
			...
			case DataCaching:
				...
				else if (Constants.PRELOADER_MIN_STAGE_TIME > 0)
				{
					...
					{
						if (TP.instance.cachingDataPercent < (elapsedCachingData / Constants.PRELOADER_MIN_STAGE_TIME))
			...
			case SpritesheetParsing:
				...
				else if (Constants.PRELOADER_MIN_STAGE_TIME > 0)
				{
					...
					{
						if (TP.instance.parsingSpritesheetsPercent < (elapsedParsingSpritesheets / Constants.PRELOADER_MIN_STAGE_TIME))
			...
			case StageParsing:
				...
				else if (Constants.PRELOADER_MIN_STAGE_TIME > 0)
				{
					...
					{
						if (TP.instance.parsingStagesPercent < (elapsedParsingStages / Constants.PRELOADER_MIN_STAGE_TIME))
			...
			case CharacterParsing:
				...
				else if (Constants.PRELOADER_MIN_STAGE_TIME > 0)
				{
					...
					{
						if (TP.instance.parsingCharactersPercent < (elapsedParsingCharacters / Constants.PRELOADER_MIN_STAGE_TIME))
